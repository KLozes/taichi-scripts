...
W = ti.Vector(4, dt=real, shape=(N,N)) # cell average primitive variables vector [rho, u, v, p]
...
def is_interior_x_face(i,j):
    return i > 1 and i < N-1 and j > 0 and j < N-1

@ti.func
def is_boundary_x_face(i,j):
    return (i == 1 or i == N-1) and j > 0 and j < N-1

@ti.func
def is_interior_y_face(i,j):
    return i > 0 and i < N-1 and j > 1 and j < N-1

@ti.func
def is_boundary_y_face(i,j):
    return i > 0 and i < N-1 and (j == 1 or j == N-1)
...
@ti.kernel
def update(rk_step: ti.template()):
    for i,j in Q:
